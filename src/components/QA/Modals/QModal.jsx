...
function QModal({ prodInfo, setOpenQModal, axPost }) {
  ...
  return (
    <div className="flex justify-center items-center overflow-x-hidden overflow-y-auto fixed inset-0 z-50 shadow-lg outline-none focus:outline-none">
      <form data-testid="qmod-form" className="bg-gray-200 shadow-md rounded px-8 pt-6 pb-8 w-full outline-none focus:outline-none" onSubmit={handleQModalClick}>
        <div data-testid="qmod-title" className="text-3xl font-semibold">{`About the ${prodInfo.name}`}</div>
        ...
        <div className="flex items-center justify-around pt-6">
          <button data-testid="qmod-cancel-btn" className="border-[1px] border-slate-600 font-semibold uppercase text-sm p-4 rounded-sm shadow-inner mr-1 mb-1" type="button" onClick={() => setOpenQModal(false)}>Cancel</button>
          <button data-testid="qmod-submit-btn" className="border-[1px] border-slate-600 font-semibold uppercase text-sm p-4 rounded-sm shadow-inner mr-1 mb-1" type="submit">Submit Question</button>
        </div>
      </form>
    </div>
  );
